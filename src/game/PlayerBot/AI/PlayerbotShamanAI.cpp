...
CombatManeuverReturns PlayerbotShamanAI::DoNextCombatManeuverPVE(Unit* pTarget)
{
    ...
    switch (spec)
    {
        case SHAMAN_SPEC_ENHANCEMENT:
            if (STORMSTRIKE > 0 && (m_bot.IsSpellReady(STORMSTRIKE)) && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_NATURE) && m_ai.CastSpell(STORMSTRIKE, *pTarget) == SPELL_CAST_OK)
                return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotShamanAI::HealPlayer(Player* target)
{
    ...
    if (target->GetHealthPercent() < 30 && HEALING_WAVE > 0 && m_ai.CastSpell(HEALING_WAVE, *target) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
    if (target->GetHealthPercent() < 50 && CHAIN_HEAL > 0 && m_ai.CastSpell(CHAIN_HEAL, *target) == SPELL_CAST_OK)
        return RETURN_CONTINUE;	//JIFEDIT //prefer chain heal over healing wave on medium low health
    if (target->GetHealthPercent() < 70 && LESSER_HEALING_WAVE > 0 && m_ai.CastSpell(LESSER_HEALING_WAVE, *target) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
...
if( m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_TANK ){
    if (STONECLAW_TOTEM > 0 && !pTarget->HasAura(STONECLAW_TOTEM, EFFECT_INDEX_0) && m_ai.GetHealthPercent() < 30 && m_ai.CastSpell(STONECLAW_TOTEM) == SPELL_CAST_OK)
        return;	//if tanking and in truble throw down a stoneclaw
};
...
if(m_ai.GetAttackerCount() >= 2){
    ...
    if ((water == nullptr) || (m_bot.GetDistance(water) > 30))
    {
        if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_FIRE && FIRE_RESISTANCE_TOTEM > 0 && m_ai.CastSpell(FIRE_RESISTANCE_TOTEM) == SPELL_CAST_OK)
            return;
    if (HEALING_STREAM_TOTEM > 0 && m_ai.GetHealthPercent() < 80 && !m_bot.HasAura(HEALING_STREAM_TOTEM, EFFECT_INDEX_0) && !m_bot.HasAura(MANA_SPRING_TOTEM, EFFECT_INDEX_0) && m_ai.CastSpell(HEALING_STREAM_TOTEM) == SPELL_CAST_OK)
        return;
    ...
    /*if (FIRE_NOVA_TOTEM > 0 && m_ai.CastSpell(FIRE_NOVA_TOTEM) == SPELL_CAST_OK)
        return RETURN_CONTINUE;*/
    /*if (GROUNDING_TOTEM > 0 && !m_bot.HasAura(GROUNDING_TOTEM, EFFECT_INDEX_0) && !m_bot.HasAura(GRACE_OF_AIR_TOTEM, EFFECT_INDEX_0) && !m_bot.HasAura(WIND_FURY_TOTEM, EFFECT_INDEX_0) && m_ai.CastSpell(GROUNDING_TOTEM) == SPELL_CAST_OK)
        return RETURN_CONTINUE;*/
